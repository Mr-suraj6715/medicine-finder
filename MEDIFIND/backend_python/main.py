...
def get_shop_orders(pharmacyId: str = Query(...), db: Session = Depends(get_db)):
    order_items = db.query(models.OrderItem).options(
    ...
    for item in order_items:
        ...
        order_map[o.id]["total"] += item.priceAtTime * item.quantity
...
def ai_prescribe(req: schemas.AIPrescribeRequest):
    return {
        "extractedMedicines": ["Paracetamol 500mg", "Amoxicillin 250mg"],
